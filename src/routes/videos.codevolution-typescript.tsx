...
export const Route = createRoute({
  ...
  path: '/videos/codevolution-typescript',
  head: () => buildPageMeta({
    title: 'React TypeScript – Codevolution Video Tutorials',
    description: '26 videos · Complete React TypeScript guide by Codevolution — typing props, hooks, context, and more.',
    path: '/videos/codevolution-typescript',
  }),
  component: CodevolutionTypeScriptVideos,
})

function CodevolutionTypeScriptVideos() {
  const [query, setQuery] = useState('')

  const filtered = useMemo(() => {
    return codevolutionTypeScriptVideos.filter((v) => {
  ...
  return (
    <div className="flex flex-col flex-1 p-6 max-w-6xl mx-auto w-full gap-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-semibold">React TypeScript by Codevolution</h1>
        <p className="text-muted-foreground text-sm mt-1">
          {codevolutionTypeScriptVideos.length} videos · Complete React TypeScript guide — typing props, hooks, context, and more
        </p>
      </div>
