...
export const Route = createRoute({
  ...
  path: '/videos/striver-dsa',
  head: () => buildPageMeta({
    title: "Striver's A2Z DSA Course - Video Tutorials",
    description: `${striverDSAVideos.length} videos - Complete DSA course by Striver covering arrays, linked lists, trees, graphs, DP, and more.`,
    path: '/videos/striver-dsa',
  }),
  component: StriverDSAVideos,
})

function StriverDSAVideos() {
  const [query, setQuery] = useState('')

  const filtered = useMemo(() => {
    return striverDSAVideos.filter((v) => {
  ...
  return (
    <div className="flex flex-col flex-1 p-6 max-w-6xl mx-auto w-full gap-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-semibold">Strivers A2Z DSA Course</h1>
        <p className="text-muted-foreground text-sm mt-1">
          {striverDSAVideos.length} videos - Complete DSA course by Striver - arrays, linked lists, trees, graphs, DP, and more
        </p>
      </div>
